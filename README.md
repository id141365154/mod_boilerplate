...
В ответах получаем json. Json парситься и добавляеется на страницу. 
...
Если кег есть, то добавляем его на страницу.
Далее делается запрос на сервер. В ответе присутствует значение хеш суммы для js или css ресурсов данной страницы.
Это значение сравнивается со значением сохраненным в localStorage. Если хеши отличаються, то стили страницы обновляються ресурсами этого ответа и кэш перезаписывается.
...
Основные стили бедут добавлены в самое начало для всех страниц проекта.
